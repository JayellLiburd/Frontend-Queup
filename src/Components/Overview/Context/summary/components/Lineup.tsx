...
function Lineup(props: BusinessTypes) {
  ...
  useEffect(() => {
    ...
    setIsloading(true)
    axios.post(process.env.REACT_APP_Server + '/MyQueues/Que', {line: props.line_id}, {withCredentials: true}).then((res: any) => {
      if (res.length > 0) {
        setQue(res.data)
      }
    })
    setIsloading(false)
  }, [refresh, props.line_id])

  useEffect(() => {
    let queArray = [] as any
    const db = getFirestore();
    onSnapshot(query(collection(db, `Queue/${props.line_id}/Que`), orderBy('accTime', 'asc')), (snap: any) => {
  ...
  }, [counter, counter2, props.line_id])
  ...
  const CALL = (e: any, user: any) => {
    ...
    if (user.value.called === 0) {
      axios.post(process.env.REACT_APP_Server + '/queup', {line: props.line_id, user: user.id, called: user.value.called }, {withCredentials: true})
      .then((_: any) => {
        if (_.message) {
          ...
          const timer = setInterval(() => {
            ...
            if (width < 0) {
              ...
              axios.post(process.env.REACT_APP_Server + '/queup', {line: props.line_id, user: user.id, called: 1}, {withCredentials: true})
            }
          }, 100);
        }
  ...
  const remove = (e: any, user: any) => {
    axios.post(process.env.REACT_APP_Server + '/queup', {line: props.line_id, user: user.id, remove: true}, {withCredentials: true})
    .then((_: any) => {
      if (!_.message) { alert('There was an error removing this user') }
    })
  ...
  return (
    <Wrapper>
      <p style={{ textDecoration: 'underline', margin: '.5rem 0' }}>Current Line Count: {Que.length}</p>
...
const Wrapper = styled.div`
  ...
  .spot{
    ...
    position: relative;
    h2{ width: 40%; margin: 0; margin-top: -.5rem; text-indent: 1rem; font-size: .8rem; font-weight: 400;}
