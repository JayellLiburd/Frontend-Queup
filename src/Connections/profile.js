...
export function updateprofile(a,b,c,d,e) {
    ...
    axios.get('http://localhost:4000/verify/pro', {withCredentials: true}).then((response) => {

        if (!response.data.message) {
        axios.post('http://localhost:4000/' + (response.data[0].user_id) + "/profile",  new URLSearchParams({
