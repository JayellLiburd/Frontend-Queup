...
export function updateprofile(a,b,c,d,e) {
    ...
    axios.get('https://api.queueupnext.com/verify/pro', {withCredentials: true}).then((response) => {

        if (!response.data.message) {
        axios.post('https://api.queueupnext.com/auth/' + (response.data[0].user_id) + "/profile", {
